...
pub fn stdlib_lookup(module: &str, item: &str) -> Option<StdSig> {
    ...
    Some(match (module, item) {
        ...
        // সত্যিকারের মনোটোনিক ক্লক (`std::time::Instant`-ভিত্তিক) — process
        // শুরুর পর থেকে সেকেন্ড, epoch-সেকেন্ড নয়। বেঞ্চমার্কিং-এর জন্য: দুটো
        // রিডিং বিয়োগ করলে ঘড়ি অ্যাডজাস্টমেন্টেও কখনো উল্টো/নেগেটিভ ফল
        // আসবে না, যা `SystemTime`-ভিত্তিক হলে হতে পারত।
        ("সময়", "সেকেন্ড") => Fn(vec![], d()),
        // ক্যালেন্ডার ব্রেকডাউন — সবই UTC (টাইমজোন-নির্ভরতা এড়াতে), বর্তমান
        // মুহূর্তের। `সেকেন্ড_অংশ` ইচ্ছাকৃতভাবে `সেকেন্ড`-এর থেকে আলাদা নাম —
        // ওটা `মিনিট`-এর সেকেন্ড-অংশ, উপরের মনোটোনিক ক্লক নয়।
        ("সময়", "বছর") => Fn(vec![], i()),
        ("সময়", "মাস") => Fn(vec![], i()),
        ("সময়", "দিন") => Fn(vec![], i()),
        ("সময়", "ঘণ্টা") => Fn(vec![], i()),
        ("সময়", "মিনিট") => Fn(vec![], i()),
        ("সময়", "সেকেন্ড_অংশ") => Fn(vec![], i()),
        // ০ (রবিবার) .. ৬ (শনিবার)।
        ("সময়", "সপ্তাহের_দিন") => Fn(vec![], i()),
        ("সময়", "বর্তমান_তারিখ_লেখা") => Fn(vec![], s()),
        // উপরেরটার মতোই ফরম্যাট, কিন্তু "এখন"-এর বদলে দেওয়া
        // epoch-মিলিসেকেন্ড মুহূর্তের জন্য — যেমন
        // `ফাইলসিস্টেম.পরিবর্তনের_সময়()`-এর রিটার্ন ভ্যালু পড়ার-যোগ্য করতে।
        ("সময়", "তারিখ_লেখা") => Fn(vec![i()], s()),
        ("সময়", "ঘুমাও") => Fn(vec![i()], n()),
        ...
        // উপবৃত্ত/আর্ক/সেক্টর — বৃত্ত/ভরাট_বৃত্তের rx==ry বিশেষ ক্ষেত্রের
        // সাধারণীকরণ (GDI-এর Ellipse()/Arc()/Pie() নিজেই bounding-box-ভিত্তিক,
        // বৃত্তে সীমাবদ্ধ নয়)। কোণ ডিগ্রিতে সংখ্যা (int), বাকি সব
        // গ্রাফিক্স ফাংশনের মতো — জ্যামিতি দশমিক/রেডিয়ান ব্যবহার করে,
        // মিশ্রিত হলে ব্যবহারকারীকে স্পষ্টভাবে রূপান্তর করতে হয়।
        ("গ্রাফিক্স", "উপবৃত্ত") => Fn(vec![i(), i(), i(), i()], n()),
        ("গ্রাফিক্স", "ভরাট_উপবৃত্ত") => Fn(vec![i(), i(), i(), i()], n()),
        ("গ্রাফিক্স", "আর্ক") => Fn(vec![i(), i(), i(), i(), i()], n()),
        ("গ্রাফিক্স", "সেক্টর") => Fn(vec![i(), i(), i(), i(), i()], n()),
        ("গ্রাফিক্স", "ভরাট_সেক্টর") => Fn(vec![i(), i(), i(), i(), i()], n()),
        // পথ/বহুভুজ/ভরাট_বহুভুজ — দশমিক[][] শুধু (জ্যামিতি.নিয়মিত_বহুভুজ/
        // উপবৃত্ত_বিন্দুর ঠিক আউটপুট আকৃতি)। সংখ্যা[][] নেয় না ইচ্ছাকৃতভাবে:
        // রানটাইম প্রতিটি বিন্দুর ভেতরের f64 বিটপ্যাটার্ন সরাসরি পড়ে, তাই
        // সংখ্যা (i64) দিলে ভুল বিট পড়ে ভুল স্থানাঙ্ক আঁকা হতো — সাইলেন্ট
        // ভুল, কম্পাইল-টাইম টাইপ ত্রুটির বদলে।
        ("গ্রাফিক্স", "পথ") => Fn(vec![ma()], n()),
        ...
        // জ্যামিতি — বিন্দু একটি ২-দৈর্ঘ্যের দশমিক[] ([x, y]), বহুভুজ একটি
        // দশমিক[][] (বিন্দুর তালিকা) — ম্যাট্রিক্সের ভেক্টর/ম্যাট্রিক্স কনভেনশন
        // অনুসরণ করে, যাতে জ্যামিতির আউটপুট সরাসরি অন্য জ্যামিতি/ম্যাট্রিক্স
        // কলে বা লুপে ব্যবহারযোগ্য হয়।
        ("জ্যামিতি", "দূরত্ব") => Fn(vec![d(), d(), d(), d()], d()),
        ("জ্যামিতি", "কোণ") => Fn(vec![d(), d(), d(), d()], d()),
        ("জ্যামিতি", "ঘোরানো") => Fn(vec![d(), d(), d(), d(), d()], da()),
        ("জ্যামিতি", "বৃত্তের_ক্ষেত্রফল") => Fn(vec![d()], d()),
        ("জ্যামিতি", "বৃত্তের_পরিধি") => Fn(vec![d()], d()),
        ("জ্যামিতি", "উপবৃত্তের_ক্ষেত্রফল") => Fn(vec![d(), d()], d()),
        ("জ্যামিতি", "উপবৃত্তের_পরিধি") => Fn(vec![d(), d()], d()),
        ("জ্যামিতি", "ত্রিভুজের_ক্ষেত্রফল") => Fn(vec![d(), d(), d(), d(), d(), d()], d()),
        ("জ্যামিতি", "বহুভুজের_ক্ষেত্রফল") => Fn(vec![ma()], d()),
        ("জ্যামিতি", "গোলকের_আয়তন") => Fn(vec![d()], d()),
        ("জ্যামিতি", "গোলকের_পৃষ্ঠফল") => Fn(vec![d()], d()),
        ("জ্যামিতি", "শঙ্কুর_আয়তন") => Fn(vec![d(), d()], d()),
        ("জ্যামিতি", "শঙ্কুর_পৃষ্ঠফল") => Fn(vec![d(), d()], d()),
        ("জ্যামিতি", "সিলিন্ডারের_আয়তন") => Fn(vec![d(), d()], d()),
        ("জ্যামিতি", "সিলিন্ডারের_পৃষ্ঠফল") => Fn(vec![d(), d()], d()),
        ...
        ("পরিসংখ্যান", "সমষ্টি") => Fn(vec![da()], d()),
        ("পরিসংখ্যান", "গড়") => Fn(vec![da()], d()),
        ("পরিসংখ্যান", "মধ্যক") => Fn(vec![da()], d()),
        ("পরিসংখ্যান", "প্রচুরক") => Fn(vec![da()], d()),
        ("পরিসংখ্যান", "ভেদাংক") => Fn(vec![da()], d()),
        ("পরিসংখ্যান", "আদর্শ_বিচ্যুতি") => Fn(vec![da()], d()),
        ("পরিসংখ্যান", "সহভেদাংক") => Fn(vec![da(), da()], d()),
        ("পরিসংখ্যান", "সহসম্পর্ক") => Fn(vec![da(), da()], d()),
        ("পরিসংখ্যান", "রৈখিক_রিগ্রেশন") => Fn(vec![da(), da()], da()),

        // সিস্টেম — কমান্ড-লাইন আর্গুমেন্ট ও এনভায়রনমেন্ট ভ্যারিয়েবল।
        // আর্গুমেন্ট() স্ক্রিপ্ট/এক্সিকিউটেবলের নিজের পাথ বাদ দিয়ে বাকিটা
        // ফেরত দেয় — ইন্টারপ্রেটেড (`কলম চালাও file.ক ...`) ও নেটিভ (compiled
        // .exe-এর নিজস্ব OS argv) দুই মোডেই।
        ("সিস্টেম", "আর্গুমেন্ট") => Fn(vec![], sa()),
        ("সিস্টেম", "পরিবেশ") => Fn(vec![s()], s()),
...
impl Ck {
    ...
    fn call(&mut self, name: &str, pos: Pos, args: &[Expr]) -> Ty {
        ...
        match name {
            ...
            // মূল অ্যারে অক্ষত রেখে একটি নতুন সাজানো অ্যারে ফেরত দেয় (`কপি`র
            // মতোই non-mutating) — এভাবে ownership/move-এর জটিলতা এড়ানো যায়,
            // যেটা in-place সর্ট করলে (আর্গুমেন্ট move হলো কিনা) সিদ্ধান্ত নিতে হতো।
            "সাজাও" => {
                if args.len() != 1 {
                    self.err(
                        pos,
                        format!("'সাজাও' ১টি আর্গুমেন্ট নেয়, {}টি পেয়েছে", bn_num(args.len() as u32)),
                    );
                    return Ty::Err;
                }
                let t = self.expr(&args[0]).unwrap_or(Ty::Unknown);
                match &t {
                    Ty::Arr(inner) if matches!(**inner, Ty::Num | Ty::Dec | Ty::Txt) => t,
                    Ty::Unknown | Ty::Err => Ty::Unknown,
                    other => {
                        self.err(pos, format!("'সাজাও' 'সংখ্যা[]'/'দশমিক[]'/'লেখা[]' নেয়, '{}' নয়", other));
                        Ty::Err
                    }
                }
            }
            // অ্যারে-বিল্ডিং-এর একমাত্র উপায় আগে ছিল `arr = arr + [item]` —
            // প্রতিটা কল পুরো অ্যারে কপি করে, তাই n-বার চালালে O(n²)। একটা
            // সেলফ-হোস্টেড লেক্সার/পার্সারের টোকেন-লিস্ট/AST-নোড-লিস্ট বানানোর
            // জন্য এটা বাস্তবে ব্যবহারযোগ্য না (v2-prerequisites.md §৬ দেখুন)।
            // `kl_arr_push` (amortized-growth) রানটাইমে আগে থেকেই ছিল, নেটিভ
            // কোডজেন অ্যারে-লিটারেল বানাতে এটা নিজেই ব্যবহার করে — এখানে শুধু
            // ভাষা-স্তরে উন্মুক্ত করা হলো। ইচ্ছাকৃতভাবে in-place mutate করে
            // (move করে না, `কপি`/`সাজাও`-এর মতোই) — কল-এর পরও `arr` বাইন্ডিং
            // বৈধ ও আপডেটেড থাকে, `try_move_src` লাগে না।
            "যোগ_করো" => {
                if args.len() != 2 {
                    self.err(
                        pos,
                        format!("'যোগ_করো' ২টি আর্গুমেন্ট নেয়, {}টি পেয়েছে", bn_num(args.len() as u32)),
                    );
                    return Ty::Err;
                }
                let at = self.expr(&args[0]).unwrap_or(Ty::Unknown);
                let it = self.expr(&args[1]).unwrap_or(Ty::Unknown);
                match &at {
                    Ty::Arr(elem) => {
                        if !unify(elem, &it) {
                            self.err(
                                args[1].pos,
                                format!("'যোগ_করো'-এর দ্বিতীয় আর্গুমেন্ট '{}' প্রত্যাশিত, '{}' পাওয়া গেছে", elem, it),
                            );
                        }
                    }
                    Ty::Unknown | Ty::Err => {}
                    other => {
                        self.err(args[0].pos, format!("'যোগ_করো' একটা অ্যারে নেয়, '{}' নয়", other));
                    }
                }
                Ty::Null
            }
            ...
            // `অক্ষর` আর তার ইউনিকোড স্কেলার ভ্যালুর মধ্যে রূপান্তর। এই
            // জোড়াটা ছাড়া কলমে কোডপয়েন্ট নিয়ে কোনো কাজ করা যায় না —
            // `\u{...}` এস্কেপ ডিকোড করা, বা অক্ষরকে রেঞ্জ-গণিতে ফেলা।
            // সেলফ-হোস্টেড লেক্সারের দুটোই লাগে। ফ্রিজ-নিয়মে (§৬) এটা
            // "সীমাবদ্ধতা শিথিল করা" — আগের কোনো বৈধ প্রোগ্রাম অবৈধ হয় না।
            "কোডে" | "অক্ষরে" => {
                if args.len() != 1 {
                    self.err(
                        pos,
                    ...
                    return if name == "কোডে" { Ty::Num } else { Ty::Ch };
                }
                let t = self.expr(&args[0]).unwrap_or(Ty::Unknown);
                let want = if name == "কোডে" { Ty::Ch } else { Ty::Num };
                if !poisoned(&t) && t != want {
                    self.err(pos, format!("'{}' '{}' নেয়, '{}' নয়", name, want, t));
                }
                if name == "কোডে" { Ty::Num } else { Ty::Ch }
            }
