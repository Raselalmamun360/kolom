...
impl Gen {
    ...
    fn call_stdlib(
        ...
        match (module, item) {
            ...
            // জ্যামিতি — এই legacy ব্যাকএন্ডে কোনো বিনিয়োগ করা হয়নি
            // (standalone নেটিভ কম্পাইলার ও ইন্টারপ্রেটারই কলমের আসল দুই
            // এক্সিকিউশন পাথ; `--সি` legacy/non-default)। ম্যাট্রিক্সের মতোই
            // পুরো মডিউল এখানে অসমর্থিত — সব ব্যাকএন্ডে parity বজায় রাখতে
            // আংশিক (শুধু স্কেলার) বাস্তবায়ন করার চেয়ে স্পষ্টভাবে না-করা ভালো।
            ("জ্যামিতি", other) => {
                panic!("জ্যামিতি.{other}: --সি (legacy) ব্যাকএন্ডে সমর্থিত নয়, ডিফল্ট Cranelift ব্যাকএন্ড অথবা ইন্টারপ্রেটার ব্যবহার করুন")
            }
            ...
            // ম্যাট্রিক্স-এর runtime ফাংশনগুলো নেস্টেড অ্যারে (দশমিক[][]) আর
            // Gaussian elimination-ভিত্তিক অ্যালগরিদম ব্যবহার করে —
            // kolom-runtime-এ (Cranelift ব্যাকএন্ড) লেখা, legacy `--সি`
            // ব্যাকএন্ডের হাতে-লেখা C কোডজেনে পোর্ট করা হয়নি। Wildcard `_ =>
            // "(0)"` -এর মধ্যে পড়ে silently ভুল মান দেওয়ার বদলে স্পষ্ট প্যানিক।
            ("ম্যাট্রিক্স", other) => {
                panic!("ম্যাট্রিক্স.{other}: --সি (legacy) ব্যাকএন্ডে সমর্থিত নয়, ডিফল্ট Cranelift ব্যাকএন্ড ব্যবহার করুন")
            }
